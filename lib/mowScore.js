...
export function groupGoodWindows(scoredHours, minScore = 70, daysSinceMowed = null) {
  const now = new Date();
  ...
  .map((window) => {
    ...
    if (window.hours.length <= maxHours) {
      ...
      weatherScore =
        chosenHours.reduce((sum, h) => sum + h.score, 0) / chosenHours.length;
    } else {
    ...
    const finalScore = Math.round(weatherScore + needSoonBonus - delayPenalty);
