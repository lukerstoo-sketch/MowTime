...
export function groupGoodWindows(
  ...
  function isWithinRange(itemDate, searchRange) {
    ...
    const diffDays = Math.round(
      (itemMidnight - todayMidnight) / (1000 * 60 * 60 * 24)
    );
  
    if (searchRange === "2days") return diffDays >= 0 && diffDays <= 2;
    if (searchRange === "5days") return diffDays >= 0 && diffDays <= 5;
    if (searchRange === "7days") return diffDays >= 0 && diffDays <= 7;
  
    if (searchRange === "weekend") {
      const day = itemDate.getDay(); // 0 = Sun, 6 = Sat
      return day === 0 || day === 6;
    }
  ...
  const candidateHours = scoredHours.filter((item) => {
    ...
    return (
      itemDate > now &&
      isWithinRange(itemDate, searchRange) &&
      item.score >= minScore &&
      hour >= 10 &&
      hour <= 19
    );
